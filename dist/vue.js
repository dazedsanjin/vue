...
(function (global, factory) {
  ...
  function observe (value, asRootData) {
    ...
    var ob;
    // 如果 value 对象上存在 __ob__ 属性，则表示已经做过观察，直接返回 __ob__ 属性
  ...
  function defineReactive (
    ...
    Object.defineProperty(obj, key, {
      ...
      get: function reactiveGetter () {
        ...
        // 读取时进行依赖收集，将 dep 添加到 watcher 中，也将 watcher 添加到 dep 中
        if (Dep.target) {
          dep.depend();
          if (childOb) {
            // 对嵌套对象依赖收集
  ...
  function initState (vm) {
    ...
    var opts = vm.$options;
    // 对 props 配置做响应式处理
    // 代理 props 配置上的 key 到 vue 实例，支持 this.prop 的方式访问
    if (opts.props) { initProps(vm, opts.props); }
    // 判重处理，methods 的方法名称不能和 props 中的属性重复，props > methods
    // 将 methods 中的方法赋值到 vue 实例上，支持通过 this.method 的方式访问
    if (opts.methods) { initMethods(vm, opts.methods); }
    // 判重处理，data 中的属性不能和 props 和 methods 的方法重复
    // 代理，将 data 中的属性代理到 vue 的实例上
    // 响应式处理
    if (opts.data) {
      initData(vm);
    } else {
    ...
    // computed 是通过 watcher 来实现的，对每个 computedKey 实例化一个 watcher，默认懒执行（动态加载）
    // 将 computedKey 代理到 vue 实例上
    // computed缓存机制，主要利用 watcher.dirty来控制 watcher.evaluate() 置为 false ，数据更新后再置为 true
  ...
  function createComputedGetter (key) {
    ...
    return function computedGetter () {
      ...
      if (watcher) {
        if (watcher.dirty) {
          // 获取 computed 函数计算的返回值
          // 待页面更新后，再将 watcher.dirty 置为 true
          watcher.evaluate();
        }
        if (Dep.target) {
          watcher.depend();
  ...
  function initWatch (vm, watch) {
    ...
    for (var key in watch) {
      ...
      // handler 为数组，遍历数组，获取其中的每一项，并创建对应的 Watcher
  ...
  function createWatcher (
    ...
    // 如果 handler 为字符串，这说明是一个 methods 方法，获取vm[handler]
